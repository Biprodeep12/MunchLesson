...
export default function Nav() {
  ...
  return (
    <>
      <nav className='bg-[#04285a] items-center flex pt-[10px] pb-[15px] sm:px-10 px-5 gap-5'>
        <Image src='/studymunch_logo.jpg' width={65} height={65} alt='Logo' />
        <div className='sm:text-3xl text-2xl font-bold text-white'>
          StudyMunch
        </div>

        <div className='text-xl text-gray-400 md:flex flex-row gap-5 ml-auto font-bold hidden items-center'>
          {['Home', 'Dashboard', 'Credits'].map((name, index) => (
          ...
          {user ? (
            ...
            <Link href='/AuthPage' className='cursor-pointer hover:text-white'>
              Login
            </Link>
          )}
        </div>
